...
function showFeedback(title) {
  ...
  // Add a button for giving feedback
...
// Function to retrieve feedback data from local storage
...
function showFeedbackForm(title) {
  ...
  form.onsubmit = function (event) {
    ...
    // Add the feedback to the recipe
    const recipe = recipes.find(recipe => recipe.title === title);
...
// Function to load feedback data from local storage and populate the recipes
...
// Define an array to store favorite recipes
let favoriteRecipes = [];
...
// Function to update the favorite button in the navbar
function updateFavoriteButton() {
  const favButton = document.getElementById('favButton');
  const favoriteCount = favoriteRecipes.length;
  favButton.innerHTML = `Favorites (${favoriteCount})`;

  // Apply CSS transition to smoothly change button color
...
function toggleFavorite(event, title) {
  ...
  if (index === -1) {
    ...
    favoriteRecipes.push(recipe);
    event.target.style.backgroundColor = "red"; // Change button color to red
  } else {
    // If already in favorites, remove it
    favoriteRecipes.splice(index, 1);
    event.target.style.backgroundColor = ""; // Revert button color
  }

  // Update the favorite button count
  updateFavoriteButton();
}

// Event listener for the navbar favorite button
